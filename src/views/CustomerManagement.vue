            ...
            >
              <button @click="nextPage" class="page-link">Next</button>
                    ...
                    <Modal v-if="showModal" @close="showModal = false">
                            <AddCustomer @close="showModal= false"/>
                    </Modal>
                    <Modal v-if="showDetail" @close="showDetail= false">
                            <DetailCustomer :detail="detail"/>
                    </Modal>
                    <Modal v-if="showEdit" @close="showEdit= false">
                            <EditCustomer :detail="detail" :index="index" @close="showEdit= false"/>
                    </Modal>
                    <Modal v-if="showDelete" @close="showDelete= false">
                            <DeleteCustomer :detail="detail" :index="index"  @close="showDelete= false"/>
                    </Modal>
            </div>
        </div>
    </div>
...
const totalPages = computed(() => {
  const totalItems = CusData.customers.data.length;
...
const currentPageData = computed(() => {
  ...
  return CusData.customers.data.slice(startIndex, endIndex);
});
...
function handleDelete(item, idx) {
    ...
    detail.value = item;
    index.value = (currentPage.value - 1) * itemsPerPage.value + idx ;
}
